...
test("maps Korean Notion properties and ignores formulas", () => {
  const movie = mapNotionPageToMovie({
    id: "movie",
    properties: {
      이름: {
        type: "title",
        title: [{ plain_text: "Film " }, { plain_text: "Stock" }],
      },
...
test("sorts newest watched dates first, missing dates last, without mutation", () => {
  ...
  const movies = [
    base,
    { ...base, id: "older", watchedDate: "2025-01-01" },
    { ...base, id: "newer", watchedDate: "2026-01-01" },
  ];
  assert.deepEqual(
    sortMoviesByWatchedDate(movies).map((movie) => movie.id),
    ["newer", "older", "blank"],
  );
...
test("paginates beyond 100 rows and continues through an empty result page", async () => {
  ...
  const result = await collectPages(async (cursor) => {
    calls.push(cursor);
    if (!cursor)
      return { results: rows, has_more: true, next_cursor: "second" };
    if (cursor === "second")
      return { results: [], has_more: true, next_cursor: "third" };
